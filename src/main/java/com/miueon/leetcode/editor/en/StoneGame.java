...
public class StoneGame {
    public static final Solution solution = new StoneGame().new Solution();
    ...
    public boolean proxy(int[] piles) {
    ...
    class Solution {
        ...
        public boolean stoneGame(int[] piles) {
            this.piles = piles;
            dpT = new int[piles.length + 2][piles.length + 2];
        ...
        public int dp(int i, int j) {
            ...
            if ((j - i - piles.length) % 2 != 0) {
                dpT[i+1][j+1] = Math.max(piles[i] + dp(i + 1, j), piles[j] + dp(i, j - 1));
                return dpT[i + 1][j + 1];
            } else {
                dpT[i+1][j+1] = Math.min(-piles[i] + dp(i + 1, j),
                        -piles[j] + dp(i, j - 1));
                return dpT[i + 1][j + 1];
            }
        }
