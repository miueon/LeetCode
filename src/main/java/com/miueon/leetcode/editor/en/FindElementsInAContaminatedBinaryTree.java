...
// 👍 193 👎 36
  ...
  public class FindElementsInAContaminatedBinaryTree{
       // public static final FindElements solution = new FindElementsInAContaminatedBinaryTree().new FindElements();
 ...
 */
class FindElements {
    private Set<Integer> quickFind = new HashSet<>();

    void bitSolve(TreeNode node, int nodeValue) {
        if (node == null) {
            return;
        }
        quickFind.add(nodeValue);
        bitSolve(node.left, (nodeValue << 1) + 1);
        bitSolve(node.right, (nodeValue<<1)+2);
    }
    ...
    public boolean find(int target) {
        return quickFind.contains(target);
    }
}
